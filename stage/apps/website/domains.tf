...
locals {
  app_domains    = [data.aws_route53_zone.app.name, format("www.%s", data.aws_route53_zone.app.name)]
  vanity_domains = local.has_vanity_domain ? [data.aws_route53_zone.vanity.name, format("www.%s", data.aws_route53_zone.vanity.name)] : []
}
...
resource "aws_route53_record" "vanity_www" {
  count = local.has_vanity_domain ? 1 : 0
